...
var formats = {
    ...
    javascriptWindow: {
        ...
        format: function (locales, options) {
            ...
            var localeName;
            locales.forEach(function (locale) {
                if (localeName !== undefined && localeName != locale.name) {
                    throw new Error("More than one locale, only one is supported at a time " + localeName + " and " + locale.name);
                }
                _.assign(result, locale.strings);
                localeName = locale.name;
            });
            return "window.localizationData = " + JSON.stringify(result, null, 4) + ";";
        }
    }
};
